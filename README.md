...
- **像素密度档位**：支持 8×8 / 16×16 / 32×32 / 64×64 / 128×128 / 256×256 / 512×512 档位（最长边对齐）。
- **默认双轮生成 (Dual-Pass)**：
  - 第一轮：模型生成初始像素草稿。
  - 第二轮：本地网格化降采样后作为提示输入，模型二次精修轮廓与色块结构。
  - 容错机制：第二轮异常时自动平滑回退至第一轮草稿，不中断渲染。
- **QVote 区域量化与拓扑清理**：
...
- **透明度与脱敏呈现**：完整呈现分阶段进度（Stepper）、中间草稿、耗时与脱敏后的诊断报错信息。
