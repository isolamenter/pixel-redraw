...
默认自动调用模型两次：第一轮生成初稿后，先在本地按目标网格和调色板量化，再把这张像素化初稿连同原图交给第二轮整理轮廓和色块，第二轮返回后再做一次最终量化；网页端同样如此，不需要手动挑选。`PIXEL_GENERATION_PASSES=1` 或 `--passes 1` 可切回单轮，便于对照或减少调用。第二轮失败时会使用首轮结果，报告中的 `refinement_applied` 会标明是否完成精修。需要定制第二轮指令时可设置 `PIXEL_REFINE_PROMPT`，支持 `{width}`、`{height}` 占位符。

模型图落到逻辑网格时按最近邻采样，以保留模型画出的色块边界；`--pixelize-only` 仍对普通输入做面积缩小。网页默认使用自动调色板，固定预设作为可选风格。
默认密度改为 `32x32`（在 1024×1024 输入上实际得到 128×128），减少高密度图像的零碎笔触；仍可按需要选择其他档位。
...
`--size` 表示 256×256 参考画布上的像素密度，不是强制输出尺寸。比如
1024×1024 输入选择 `64x64` 会得到 256×256；512×768 输入会按比例得到
128×192。可选密度为 8、16、32、64、128，输出会保留原图比例。
...
- **上传 / 粘贴 / 拖拽**三选一，上传时保留原图比例，不再中心裁剪成正方形；服务端按 256×256 参考画布计算实际像素网格。
- **像素密度档位**：8×8 / 16×16 / 32×32 / 64×64 / 128×128（这是 256 基准档位，实际输出尺寸随原图比例变化）。
- **自动色数**：8 / 12 / 16 / 24 / 32 / 48 / 64 色；64 色适合高密度输出，低密度图建议 8–24 色。
- **调色板**：15 个内置预设（4/8/16/32/46/64 色，含像素艺术和 CGA/MSX 硬件色板），网页按色数从少到多显示；或自己填 hex，或 `auto`。**选了固定调色板后输出像素严格只从该调色板取色**——这一条有断言保护，不是尽力而为。
- **改尺寸 / 换调色板不重新调用模型**：跑完一次后，会直接对缓存的模型输出重新像素化（约 0.15 秒，零 token）。
- **显示中间结果**：第二轮开始前，页面会展示第一轮 AI 输出经过 Pillow 量化后的逻辑像素图，并提供下载。
- **复制 / 下载**：复制的是放大后的预览图（直接把像素图粘进 Slack/Discord 可能被对方平滑重采样，硬边就没了）；要原始 1:1 像素请用下载。
...
| `<run_id>.draft.png` | 第一轮 AI 输出经 Pillow 量化后的中间图 |
...
```bash
.venv/bin/python3 -m unittest discover -s tests -v  # 已提交的双轮生成/失败回退测试，不调用 API
...
PIXEL_WEB_LIVE_TEST=1 python3 test/test_web.py EndToEnd   # 默认两次真实模型调用
```
