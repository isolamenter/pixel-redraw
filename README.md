...
通过 `.env` 配置 NewAPI（Gemini 网关），以 Gemini 原生 `generateContent` 协议调用支持图像输入和图像输出的模型，再在本地用 Pillow 将结果固定为真实像素画。
...
cp .env.example .env
# 编辑 .env，至少填写 NEWAPI_BASE_URL、NEWAPI_API_KEY、NEWAPI_MODEL
python3 pixel_redraw.py input.png
```

`.env` 支持引用已有环境变量，例如 `NEWAPI_API_KEY=${NEWAPI_API_KEY}`；程序不会把 Key 的实际值写回文件。
...
```
POST {NEWAPI_BASE_URL}/{NEWAPI_API_VERSION}/models/{NEWAPI_MODEL}:generateContent
x-goog-api-key: <API key>
```

- `NEWAPI_BASE_URL` 填网关根地址（如 `http://127.0.0.1:3000`），**不要**带 `/v1beta`；版本由 `NEWAPI_API_VERSION` 控制，默认 `v1beta`。
...
- 若模型支持指定出图分辨率，可设置 `NEWAPI_IMAGE_SIZE=1K|2K|4K`。
...
页面不提供输入框，任何路由都不会读取请求里的 key/model，`/api/meta` 只回 `upstream_configured: true/false` 和网关主机名（不回完整 URL，因为 URL 可能带 userinfo）。
...
- **超时不是墙钟**。`NEWAPI_TIMEOUT` 是逐 socket 超时，不是总时限，所以前端不会、也不应该根据「已等待超过 180 秒」判定失败。
...
```bash
python3 test/test_web.py            # 32 项：调色板子集 / 密钥不泄漏 / 校验 / 本地管线 / 终止帧形状
