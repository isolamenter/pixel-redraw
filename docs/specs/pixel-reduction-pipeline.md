...
**前端三张卡片展示标准**：
- **左侧**：**自适应参考 Guide (`guide_png`)** —— 展示纯本地由原图自适应分档与 Auto 色板生成的 Pass 1 参考基准图。
- **中间**：**AI 原始输出 (`raw_png`)** —— 展示大模型返回的未经二次量化的高清原始构图。
- **右侧**：**最终像素图 (`pixel_png`)** —— 展示落地到目标色板与密度的最终像素画成品。
...
- **自动调色板 (Auto Palette)**：在 Oklab 空间执行轻量、确定性（固定随机种子）的 K-Means 聚类。颜色数量根据像素网格密度（Density）自适应决定（$\ge 128 \to 64$ 色， $64 \to 48$ 色， $32 \to 32$ 色， $16 \to 16$ 色， $< 16 \to 8$ 色），兼顾大画布的色彩丰富度与小画布的像素克制感。
